...
## 2026-05-21
- `pending` — Harden startup (per-cog failures + sync failures logged not crashing; refuse start only on zero cogs loaded); intel-stream now scans rich-embed bodies, not just `message.content`, so embed-only sibling-bot posts are processed; fix latent bug where `_message_text` definition orphaned `_react_and_analyze` and `_deliver` from the cog class. Add elaborate integration test suite: `tests/fakes.py` mocks the Interaction / Message / Bot / GLM / aiohttp surface; `tests/test_cogs_integration.py` exercises every slash command and the autonomous on_message listener through fakes. Test count 25 → 48; all pass from any CWD.
