...
## 2026-05-21
- `pending` — Add keyless live OSINT enrichment (DoH DNS, RDAP, IP geo/ASN, Tor exit list); GLM receives enrichment block alongside indicators; per-user cooldowns on `/osint` + `/pivot`; pytest suite for pure services; SYSTEM_PROMPT.md updated to direct GLM to cite enrichment fields rather than caveat them.
